...
import FeaturedProductsCarouselSkeleton from "@/components/store/FeaturedProductsCarouselSkeleton";
...
export default async function Home({ searchParams }: PageProps) {
  ...
  const { data: featureProducts } = await sanityFetch({
    ...
    <div className="min-h-screen bg-zinc-50 dark:bg-zinc-900">
      ...
      {featureProducts.length > 0 && (
        <Suspense fallback={<FeaturedProductsCarouselSkeleton />}>
          <FeaturedProductsCarousel products={featureProducts} />
        </Suspense>
      )}
